...
const accumulatedRain = (dojot_host, jwt, device_id, beginning_hour, n_days_before_daily, n_days_before_hourly) => new Promise((resolve, reject) => {
  ...
  let date_before_daily = new Date();
  let date_before_hourly = new Date();
  ...
  const hour = new Date(date_before_hourly);
  for (let i = date_before_hourly.getHours() + 1; i <= hours; i++) {
    hour.setHours(i % 24);
    const next_day = Math.floor(i / 24);
    const ts = new Date(hour);
  ...
  for (let i = 0; i < n_days_before_daily - 1; i++) {
    const day = new Date(date_before_daily.setDate(date_before_daily.getDate() + 1));
    result.days.push({
      ts: new Date(day),
      value: 0,
    });
  ...
  const url = `${history}/device/${device_id}/history?attr=pcVol&dateFrom=${dateFromString}&dateTo=${dateToString}`;
  ...
  axios({
    ...
    .then((response) => {
      ...
      for (const data of response.data) {
        ...
        const ts_unix = ts.setMinutes(0, 0, 0);
